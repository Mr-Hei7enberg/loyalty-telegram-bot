...
import { LoyaltyContentService } from '../loyalty/services/loyalty-content.service';
import { DynamicCodeService } from '../loyalty/services/dynamic-code.service';
import { FeedbackService } from '../loyalty/services/feedback.service';
import type { FeedbackContactPreference } from '../loyalty/interfaces/loyalty.interface';
...
export class TelegramUpdate {
  ...
  async handleCard(@Ctx() ctx: TelegramContext) {
    ...
    try {
      const dynamicCode = await this.dynamicCodeService.getOrCreate(
        session.cardNumber,
        session.dynamicCode,
      );

      session.dynamicCode = {
        value: dynamicCode.value,
        expiresAt: dynamicCode.expiresAt,
      };
