...
import { DiscountCatalogService } from '../src/loyalty/services/discount-catalog.service';
import { RegionDirectoryService } from '../src/loyalty/services/region-directory.service';
...
async function seedReferenceData(
  sequelize: Sequelize,
  discountService: DiscountCatalogService,
  regionService: RegionDirectoryService,
) {
  await sequelize.sync();
  await discountService.getGroupsWithItems();
...
async function main() {
  ...
  try {
    ...
    const discountService = new DiscountCatalogService(
      DiscountGroup,
      DiscountItem,
    );
    const regionService = new RegionDirectoryService(
      Region,
      RegionNetwork,
      RegionLocation,
    );

    await seedReferenceData(sequelize, discountService, regionService);
